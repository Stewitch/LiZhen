...
from ..utils.bridge import Item, asrConfig
...
class ASRInterface(ManagerInterface):
    ...
    def _setCards(self):
        self.modelCard = OptionsCard(
            Item(asrConfig, "asr_model"),
            FluentIcon.MICROPHONE,
            self.tr("语音识别模型"),
            asrConfig.get_field_description("asr_model", LANG),
            AVAILABLE_VALUES["asr_model"],
            self.modelGroup
        )
        ...
        self.detailForm = SettingsForm(
            getattr(asrConfig, self.modelCard._item.value),
            self.detailGroup
        )
    ...
    def __onModelChanged(self, i):
        ...
        settings = getattr(asrConfig, model)
        logger.debug(f"ASR Model 变更: {model}")
        ...
        self.detailGroup = SettingCardGroup(self.tr("LLM详细设置"), self.view)
